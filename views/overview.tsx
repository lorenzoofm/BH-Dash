...
export default function Overview() {
  ...
  const targets = useTable("expectations"), models = useTable("models"), map = useTable("map");
  const monthStart = today ? today.slice(0, 8) + "01" : null;
  const creators = useCreators(monthStart, today);
  ...
  }, [today, staff.rows, conv.rows, pay.rows, expenses.rows, targets.rows, models.rows, map.rows, creators.data, creators.isSuccess]);
  ...
  return <div className="space-y-6">
    <PageHeader eyebrow={`Week of ${weekLabel(d.thisWeek)} · day ${d.daysIn}/7`} title="Overview" subtitle={`${d.activeCount} active staff · month to date figures from ${shortDate(d.thisWeek.slice(0, 8) + "01")}`}/>
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Panel title="Month to date" subtitle="How this month’s money breaks down" bodyClass="pt-3">
