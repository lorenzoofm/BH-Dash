...
- P&L can select several roster models at once, or all active models, and shows one combined income, payout, wages, paid expenses and profit view plus a per-model breakdown. A selected model without verifiable account revenue keeps the combined income and profit blank. Costs linked to multiple models remain divided by the existing allocation rule.
