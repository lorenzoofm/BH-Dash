...
// Use the same server-backed manager session as the editable dashboard.
      ...
      <Card className="w-full max-w-xl">
        <CardContent className="p-6">
          ...
          <p className="mt-4 text-xs text-muted-foreground">
            ...
            . Ask an administrator to review your dashboard access.
          </p>
        </CardContent>
      </Card>
    </div>
...
export default function Block() {
  ...
  if (session.isPending) return <GateResolving />;
  if (session.data?.canEdit !== true) {
    return <GateUnavailable email={session.data?.user?.email ?? ""} />;
  }
  return <BlockContent allowed={true} />;
}
