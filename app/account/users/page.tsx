...
export default function UsersPage(){
 const [emails,setEmails]=useState<string[]>([]),[admins,setAdmins]=useState<string[]>([]),[email,setEmail]=useState(''),[error,setError]=useState(''),[busy,setBusy]=useState(false),[ready,setReady]=useState(false),[invited,setInvited]=useState('');
 const load=async()=>{const response=await fetch('/api/users',{cache:'no-store'});const data=await response.json() as {error?:string;emails:string[];admins:string[]};if(!response.ok)throw new Error(data.error);setEmails(data.emails);setAdmins(data.admins);setReady(true);};
 ...
 return <div className="mx-auto max-w-2xl space-y-6"><div><h1 className="text-3xl font-semibold">Dashboard users</h1><p className="mt-2 text-sm text-muted-foreground">Invite a manager by email. Cloudflare Access verifies their identity when they open this dashboard.</p><p className="mt-2 text-sm text-muted-foreground">Managers can use operations pages. Profit &amp; Loss, Revenue, model deal terms, and finance reports require an administrator.</p></div>
